...
class PutTest < BeanstalkIntegrationTest
  ...
  context 'put' do
    ...
    should 'return BURIED and job ID if server ran out of memory trying to grow priority queue' do
      skip 'Unsure how to test server running out of memory growing queue'
    end


    should 'return DRAINING if is in drain mode' do
      skip 'Unsure how to force drain mode'
    end
